...
class MonitorHandler(FileSystemEventHandler):
    def on_any_event(self, event):
        if event.is_directory:
            return  # Skip directory events here, as we'll handle them explicitly in the process_event
    ...
    def process_event(self, file_path, event_type):
        ...
        try:
            if os.path.isfile(file_path):
                ...
                print(f"Uploaded file {file_path} to s3://{s3_bucket}/{s3_path}")
            elif os.path.isdir(file_path):
                # Create a corresponding folder in S3 by uploading a zero-byte object with a trailing slash
                if not s3_path.endswith('/'):
                    s3_path += '/'
                s3_client.put_object(Bucket=s3_bucket, Key=s3_path)
                print(f"Created folder {s3_path} in s3://{s3_bucket}/{s3_path}")
