...
# Set up logging
log_dir = "C:/logs/monitoring-box.logs"
...
class ChangeHandler(FileSystemEventHandler):
    ...
    def on_moved(self, event):
        if not event.is_directory:
            log_message(f"File moved: {event.dest_path}")
            upload_to_s3(event.dest_path)

    def on_deleted(self, event):
        # Do nothing: The script is in read-only mode and won't delete anything from source1
        log_message(f"Detected deletion of {event.src_path}, but no action taken due to read-only mode.")
...
if __name__ == "__main__":
    ...
    observer.start()
    log_message(f"Started monitoring {source1} in Read-Only mode for changes.")
